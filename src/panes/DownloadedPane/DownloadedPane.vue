...
function checkboxChecked(comic: Comic): boolean {
  return checkedIds.value.has(comic.comic.path_word)
}

function handleCheckboxClick(comic: Comic) {
  if (checkedIds.value.has(comic.comic.path_word)) {
    checkedIds.value.delete(comic.comic.path_word)
  } else {
    checkedIds.value.add(comic.comic.path_word)
  }
}

function handleContextMenu(comic: Comic) {
  if (selectedIds.value.has(comic.comic.path_word)) {
    return
  }

  selectedIds.value.clear()
  selectedIds.value.add(comic.comic.path_word)
}
...
async function exportCbz() {
  ...
  const comics = currentPageComics.value.filter((comic) => checkedIds.value.has(comic.comic.path_word))
...
function useDropdown() {
  ...
  const dropdownOptions: DropdownOption[] = [
    ...
    },
    {
      ...
      props: {
        onClick: () => {
          currentPageComics.value.forEach((comic) => selectedIds.value.add(comic.comic.path_word))
          dropdownShowing.value = false
        },
      },
    },
...
<template>
  <div v-if="store.config !== undefined" class="h-full flex flex-col overflow-auto">
    ...
    <div class="flex gap-2 items-center px-2 select-none">
      <div class="animate-pulse text-sm text-violet flex flex-col">
    ...
    <SelectionArea
      ...
      <DownloadedComicCard
        ...
        :data-key="comic.comic.path_word"
        :class="['selectable mb-2', selectedIds.has(comic.comic.path_word) ? 'selected shadow-md' : 'hover:bg-gray-1']"
        :comic="comic"
